...
# ── Material-styled helper builders ───────────────────────────────────────────
def make_card(parent, padding=12):
    """Return a card-style frame with border."""
...
def make_button(parent, text, command, style="primary", font_tuple=None, **kwargs):
...
class Tooltip:
    ...
    def show(self, _=None):
        ...
        tip_bg = P["CARD"] if P == LIGHT_PALETTE else "#1c2128"
        tip_fg = P["TEXT"]
...
# ══════════════════════════════════════════════════════════════════════════════
# ── INLINE RENAME WIDGET (theme-aware) ────────────────────────────────────────
# ══════════════════════════════════════════════════════════════════════════════
class InlineRenameEntry:
    def __init__(self, tree, item, on_confirm, on_cancel):
        ...
        self.entry.place(x=x, y=y, width=max(w, scaled(200)), height=h)
    ...
    def update(self, path):
        ...
        for i, part in enumerate(parts):
            ...
            btn = tk.Button(self, text=display,
                            bg=P["HEADER_BG"],
                            fg=P["ACCENT"] if i == len(parts) - 1 else P["SUBTEXT"],
                            activebackground=P["HOVER"],
                            activeforeground=P["ACCENT"],
                            relief="flat", bd=0, padx=4, pady=4,
                            font=("Segoe UI", scaled(9), "bold" if i == len(parts) - 1 else "normal"),
                            cursor="hand2",
                            command=lambda p=cap: self.navigate_cb(p))
            btn.pack(side="left")
            self._buttons.append(btn)
            if i < len(parts) - 1:
                tk.Label(self, text="›", bg=P["HEADER_BG"],
                         fg=P["SUBTEXT"], font=("Segoe UI", scaled(10))).pack(side="left")
...
class FileManager(tk.Tk):
    def __init__(self):
        ...
        win_w = min(1440, max(980, int(SCREEN_W * 0.88)))
        win_h = min(860, max(600, int(SCREEN_H * 0.85)))
        x_off = (SCREEN_W - win_w) // 2
        y_off = (SCREEN_H - win_h) // 2
        self.geometry(f"{win_w}x{win_h}+{x_off}+{y_off}")
        self.minsize(scaled(800), scaled(520))
    ...
    def _build_styles(self):
        ...
        # Treeview – critical for light mode readability
    ...
    def _build_ui(self):
        # ── Top bar ────────────────────────────────────────────────────────────
        topbar = tk.Frame(self, bg=P["HEADER_BG"], height=scaled(52))
        topbar.pack(fill="x", side="top")
        topbar.pack_propagate(False)

        # App logo / name
        logo_frame = tk.Frame(topbar, bg=P["HEADER_BG"])
        logo_frame.pack(side="left", padx=(10, 2), pady=6)
        tk.Label(logo_frame, text="⬡", bg=P["HEADER_BG"], fg=P["ACCENT"],
                 font=("Segoe UI", scaled(16), "bold")).pack(side="left")
        tk.Label(logo_frame, text=" NexFile", bg=P["HEADER_BG"], fg=P["TEXT"],
                 font=(self.ui_font_name, scaled(10), "bold")).pack(side="left")

        # Divider
        tk.Frame(topbar, bg=P["BORDER"], width=1).pack(side="left", fill="y", padx=6, pady=8)

        # Navigation buttons
        nav = tk.Frame(topbar, bg=P["HEADER_BG"])
        ...
        for txt, cmd, tip in nav_btns:
            b = tk.Button(nav, text=txt, bg=P["HEADER_BG"], fg=P["SUBTEXT"],
                          ...
                          relief="flat", bd=0, padx=scaled(9), pady=scaled(8),
                          font=("Segoe UI", scaled(12)), cursor="hand2", command=cmd)
            b.pack(side="left")
            Tooltip(b, tip)

        # Tabs
        self.tab_bar = tk.Frame(topbar, bg=P["HEADER_BG"])
        self.tab_bar.pack(side="left", padx=4, fill="y")
        self._rebuild_tabs()

        # Path bar
        pf = tk.Frame(topbar, bg=P["INPUT_BG"], bd=0,
                      highlightthickness=1, highlightbackground=P["BORDER"],
                      highlightcolor=P["ACCENT"])
        pf.pack(side="left", fill="x", expand=True, padx=8, pady=10)
        path_entry = tk.Entry(pf, textvariable=self.current_path,
                              bg=P["INPUT_BG"], fg=P["TEXT"],
                              insertbackground=P["ACCENT"],
                              relief="flat", font=self._f(), bd=5,
                              selectbackground=P["SEL"],
                              selectforeground=P["SEL_FG"])
        path_entry.pack(side="left", fill="x", expand=True)
        path_entry.bind("<Return>", lambda e: self.load_dir(self.current_path.get()))

        # Search
        sf = tk.Frame(topbar, bg=P["INPUT_BG"],
                      highlightthickness=1, highlightbackground=P["BORDER"],
                      highlightcolor=P["ACCENT"])
        sf.pack(side="right", padx=4, pady=10)
        tk.Label(sf, text="🔍", bg=P["INPUT_BG"], fg=P["SUBTEXT"],
                 font=self._f()).pack(side="left", padx=(6, 2))
        self.search_entry = tk.Entry(sf, textvariable=self.search_var,
                                     bg=P["INPUT_BG"], fg=P["TEXT"],
                                     insertbackground=P["ACCENT"],
                                     relief="flat", font=self._f(),
                                     width=scaled(22), bd=4,
                                     selectbackground=P["SEL"],
                                     selectforeground=P["SEL_FG"])
        self.search_entry.pack(side="left")
        self.search_entry.bind("<Return>", lambda e: self._do_search())
        self.search_var.trace_add("write", self._live_filter)

        tk.Button(sf, text="Go", bg=P["ACCENT"], fg="white",
                  activebackground=P["ACCENT2"], relief="flat", bd=0,
                  padx=8, pady=4, font=self._f(bold=True),
                  cursor="hand2", command=self._do_search).pack(side="left", padx=2)
        tk.Button(sf, text="🌐", bg=P["ACCENT2"], fg="white",
                  activebackground=P["DANGER"], relief="flat", bd=0,
                  padx=6, pady=4, font=self._f(),
                  cursor="hand2", command=self.open_system_search).pack(side="left", padx=(0, 2))

        # Support / Donate button
        support_btn = tk.Button(topbar, text="💙 Support",
        ...
        Tooltip(support_btn, "Support & Donate — Asif Miah")
        ...
        left_actions = [
            ("📁+", self.new_folder, P["ACCENT"], "New Folder (Ctrl+N)"),
            ("📄+", self.new_file, P["ACCENT"], "New File (Ctrl+Shift+N)"),
            ("📦+", self.open_bulk_create, P["ACCENT2"], "Bulk Create (Ctrl+Shift+B)"),
            ("✂", self.cut_files, P["WARNING"], "Cut (Ctrl+X)"),
            ("📋", self.copy_files, P["SUCCESS"], "Copy (Ctrl+C)"),
            ("📥", self.paste_files, P["SUCCESS"], "Paste (Ctrl+V)"),
            ("📥+", self.paste_to_multiple_dirs, P["SUCCESS"], "Paste to Multiple"),
            ("🗑", self.delete_files, P["DANGER"], "Delete (Del)"),
            ("✏", self.rename_file, P["ACCENT2"], "Rename (F2)"),
            ("↩", self.undo_action, P["SUBTEXT"], "Undo (Ctrl+Z)"),
            ("↪", self.redo_action, P["SUBTEXT"], "Redo (Ctrl+Y)"),
        ]
        for label, cmd, color, tip in left_actions:
            b = tk.Button(toolbar, text=label, bg=P["PANEL"], fg=color,
        ...
        tk.Frame(toolbar, bg=P["BORDER"], width=1).pack(side="left", fill="y", padx=4, pady=6)

        tk.Label(toolbar, text="Ext:", bg=P["PANEL"], fg=P["SUBTEXT"],
                 font=self._f()).pack(side="left", padx=(4, 2))
        fe = tk.Entry(toolbar, textvariable=self.filter_ext_var,
                      width=7, bg=P["INPUT_BG"], fg=P["TEXT"],
                      relief="flat", font=self._f(),
                      insertbackground=P["ACCENT"],
                      selectbackground=P["SEL"],
                      selectforeground=P["SEL_FG"])
        fe.pack(side="left")
        fe.bind("<Return>", lambda e: self._apply_filter_ext())
        tk.Button(toolbar, text="▶", bg=P["ACCENT"], fg="white",
                  relief="flat", padx=4, pady=3, font=self._f(),
                  command=self._apply_filter_ext).pack(side="left", padx=2)

        tk.Frame(toolbar, bg=P["BORDER"], width=1).pack(side="left", fill="y", padx=4, pady=6)

        # Add Auto-Organize Apply button here
        apply_organize_btn = tk.Button(toolbar, text="✅ Apply Org", bg=P["SUCCESS"], fg="white",
                                       activebackground="#059669", activeforeground="white",
                                       relief="flat", bd=0, padx=scaled(8), pady=scaled(7),
                                       font=self._f(), cursor="hand2",
                                       command=self.open_auto_organize)
        apply_organize_btn.pack(side="left", padx=2)
        Tooltip(apply_organize_btn, "Apply Auto-Organize (Ctrl+Shift+O)")

        right_btns = [
            ("🌙" if self.theme == "light" else "☀️", self._toggle_theme, P["TEXT"], "Toggle Theme"),
            ("📦", self.compress_selected, P["SUBTEXT"], "Zip Selected"),
            ("📂", self.extract_archive, P["SUBTEXT"], "Extract Archive"),
            ("🔗", self.create_symlink, P["ACCENT2"], "Create Symlink"),
            ("⚖️", self.compare_files, P["ACCENT2"], "Compare Files"),
            ("🏷️", self.manage_tags, P["ACCENT2"], "Manage Tags"),
            ("🎨", self.open_color_mark_toolbar, P["WARNING"], "Color Mark"),
            ("🔀", self.batch_rename, P["WARNING"], "Batch Rename"),
            ("🗂️", self.open_auto_organize, P["ACCENT"], "Auto-Organize Files (Ctrl+Shift+O)"),
            ("💻", self.open_terminal_here, P["SUBTEXT"], "Terminal Here (F4)"),
            ("📋", self.copy_selected_path, P["SUBTEXT"], "Copy Path"),
            ("🔖", self.add_bookmark, P["ACCENT2"], "Add Bookmark"),
            ("📊", self.show_props, P["SUBTEXT"], "Properties"),
            ("🔤", self.open_font_settings, P["ACCENT"], "Font Settings"),
        ]
        for label, cmd, color, tip in right_btns:
            b = tk.Button(toolbar, text=label, bg=P["PANEL"], fg=color,
                          activebackground=P["HOVER"], activeforeground=color,
                          relief="flat", bd=0, padx=scaled(6), pady=scaled(7),
                          font=self._f(), cursor="hand2", command=cmd)
            b.pack(side="right")
            Tooltip(b, tip)

        tk.Frame(toolbar, bg=P["BORDER"], width=1).pack(side="right", fill="y", padx=4, pady=6)
        tk.Button(toolbar, text="📊 Excel", bg=P["SUCCESS"], fg="white",
                  activebackground="#059669", relief="flat", bd=0, padx=8, pady=5,
                  font=self._f(bold=True), cursor="hand2",
                  command=self.open_export_dialog_excel).pack(side="right", padx=2)
        tk.Button(toolbar, text="📑 PDF", bg="#e11d48", fg="white",
                  activebackground="#be123c", relief="flat", bd=0, padx=8, pady=5,
                  font=self._f(bold=True), cursor="hand2",
                  command=self.open_export_dialog_pdf).pack(side="right", padx=2)
        tk.Frame(toolbar, bg=P["BORDER"], width=1).pack(side="right", fill="y", padx=4, pady=6)
        tk.Checkbutton(toolbar, text="Hidden", variable=self.show_hidden,
        ...
        vm_frame = tk.Frame(toolbar, bg=P["PANEL"])
        vm_frame.pack(side="right", padx=4)
        for mode, icon, tip in [("details", "≡", "List view"), ("icons", "⊞", "Icon grid")]:
            b = tk.Button(vm_frame, text=icon, bg=P["PANEL"], fg=P["SUBTEXT"],
                          activebackground=P["HOVER"], activeforeground=P["ACCENT"],
                          relief="flat", padx=6, pady=4, font=("Segoe UI", scaled(12)),
        ...
        self.breadcrumb = BreadcrumbBar(
            ...
            bg=P["HEADER_BG"], height=scaled(28))
        self.breadcrumb.pack(fill="x", side="top")

        # ── Color Mark Toolbar (hidden by default) ─────────────────────────────
        ...
        for col, label in [("name", "Name"), ("size", "Size"), ("modified", "Date"), ("type", "Type")]:
            ...
            b = tk.Button(sort_bar, text=label,
                          ...
                          relief="flat", padx=8, pady=3, font=self._f(),
                          cursor="hand2",
        ...
        # Sidebar
        sidebar = tk.Frame(main, bg=P["SIDEBAR"], width=scaled(190))
        ...
        sb_tabs.pack(fill="x")
        self._sb_frames = {}
        for key, label in [("quick", "Access"), ("recent", "Recent"), ("tags", "Tags")]:
            b = tk.Button(sb_tabs, text=label, bg=P["SIDEBAR"], fg=P["SUBTEXT"],
                          activebackground=P["HOVER"], activeforeground=P["ACCENT"],
                          relief="flat", bd=0, padx=8, pady=6, font=self._f(),
        ...
        # Preview panel
        self.preview = tk.Frame(content, bg=P["PANEL"], width=scaled(300))
        self.preview.pack(side="right", fill="y")
        self.preview.pack_propagate(False)

        preview_header = tk.Frame(self.preview, bg=P["HEADER_BG"], height=scaled(32))
        ...
        # Inline edit frame
        self.inline_edit_frame = tk.Frame(self, bg=P["PANEL"], height=0)
        self.inline_edit_frame.pack(fill="x", side="bottom")
        self.inline_edit_frame.pack_propagate(False)

        # Status bar
        statusbar = tk.Frame(self, bg=P["HEADER_BG"], height=scaled(26))
        ...
        self.sel_label = tk.Label(statusbar, text="",
                                  bg=P["HEADER_BG"], fg=P["ACCENT"], font=self._f())
        self.sel_label.pack(side="right", padx=12)
        self.watcher_label = tk.Label(statusbar,
                                      text="👁 Live" if WATCHDOG_AVAILABLE else "",
                                      bg=P["HEADER_BG"], fg=P["SUCCESS"], font=self._f())
        self.watcher_label.pack(side="right", padx=6)
    ...
    def open_auto_organize(self):
        ...
        win.geometry(f"{scaled(700)}x{scaled(680)}")
        ...
        tk.Label(hdr, text="  🗂️  Auto-Organize — Sort Files into Subfolders",
                 bg=P["HEADER_BG"], fg=P["ACCENT"],
        ...
        # Quick Apply button section (new)
    ...
    def _build_icons_view(self, entries):
        ...
        def make_icon_cell(parent, entry):
            ...
            icon_lbl = tk.Label(frame, text=icon_txt, bg=cell_bg,
                                font=("Segoe UI Emoji", scaled(28)), pady=scaled(6))
            icon_lbl.pack()
    ...
    def _build_sidebar_quick(self):
        ...
        tk.Label(f, text="QUICK ACCESS", bg=P["SIDEBAR"], fg=P["SUBTEXT"],
                 font=self._f(bold=True), pady=8).pack(anchor="w", padx=10)
        ...
        tk.Label(f, text="BOOKMARKS", bg=P["SIDEBAR"], fg=P["SUBTEXT"],
                 font=self._f(bold=True), pady=6).pack(anchor="w", padx=10)
    ...
    def _build_sidebar_recent(self):
        ...
        for path in reversed(self.recent_files[-30:]):
            ...
            btn = tk.Button(inner, text=f"  {file_icon(path)} {name[:18]}",
    ...
    def _build_sidebar_tags(self):
        ...
        for tag_name, color in self.tag_manager.colors.items():
            ...
            tk.Label(bf, text="●", bg=P["SIDEBAR"], fg=color,
                     font=("Segoe UI", scaled(11))).pack(side="left")
    ...
    def _bind_keys(self):
        ...
        self.bind("<F3>", lambda e: self.search_entry.focus())
    ...
    def _rebuild_tabs(self):
        ...
        for i, path in enumerate(self._tab_dirs):
            ...
            f.pack(side="left")
            btn = tk.Button(f, text=f"  {name[:10]}  ", bg=bg, fg=color,
        ...
        tk.Button(self.tab_bar, text="+", bg=P["HEADER_BG"], fg=P["SUCCESS"],
                  activebackground=P["HOVER"],
                  relief="flat", bd=0, padx=6, pady=8, font=(self.ui_font_name, scaled(11)),
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # ── SUPPORT / DONATE DIALOG (Redesigned) ──────────────────────────────────
    # ══════════════════════════════════════════════════════════════════════════
